...
export const compactPlateNumber = (value) => {
  ...
  const characters = Array.from(String(value).normalize('NFKC').trim());
  return characters
...
export const normalizePlateNumber = (value) => {
  ...
  return letterText || digitText || null;
};

export const normalizePlateNumberSearch = (value) => normalizePlateNumber(value) || String(value ?? '').trim();
